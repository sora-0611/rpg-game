...
function initializeUI() {
  ...
  if (btnMenuUpgrade) {
    btnMenuUpgrade.addEventListener('click', () => {
      window.location.href = '../enhancement/enhancement.html';
    });
  }
  ...
  if (btnMenuSettings) {
    btnMenuSettings.addEventListener('click', () => {
      window.location.href = '../settings/settings.html?from=explore';
    });
  }
...
function handleExploreMove(direction) {
  ...
  if (gameState.battle.isActive) {
    ...
    window.location.assign('../battle/battle.html?enemy=' + encodeURIComponent(battleEnemyId));
    return;
  }
